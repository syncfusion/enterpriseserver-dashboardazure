...
var itemUrl;
var itemLinkForSharePermission = "";
...
function getDashboardShareLink(dashboardId, categoryName, dashboardName) {
    return encodeURI(baseUrl + document.documentElement.lang + "/dashboards/" + "/" + categoryName + "/" + dashboardName);
}
...
function getLinkURL() {
    ...
    if ($("#isunlisted").val() == "true" && isOwner && (getlink.indexOf("unlistedCode") == -1)) {
        getlink += (getlink.indexOf("?") == -1 ? "?unlistedCode=" : "&unlistedCode=") + unlistedCode;
    }
    return getlink;
}