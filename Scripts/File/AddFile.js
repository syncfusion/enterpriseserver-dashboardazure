$(function () {
    ...
    $(document).on("change", "#browse_file", function () {
        ...
        if ($(this).val() != '' && IsValidUploadFile()) {            
            ...
            $("#validate-file").text("[[[Uploaded file is either an executable or a harmful file that is not supported in the Dashboard Server.]]]");
            $("#browse_file").val("");
            $('#browse_file').attr('title', $(this).val());
            isValid = false;
        }
...
function addfileValidate() {
    ...
    var extensionType = "." + $('#browse_file').val().split('.').pop();
    ValidateFile();
    if ($("#addItemForm").valid() && ValidateFile() && IsValidUploadFile()) {
        ...
        $.ajax({
            type: "POST",
            url: isFileExistUrl,
            data: { filename: fileName, extensionType: extensionType },
            async: false,
