    ...
    if (itemId != "") {
        $.ajax({
            ...
            success: function (result, data) {
                ...
                $("#selected_childdashboard").html("");
                $("#selected_childdashboard").append('<option value="all" selected=selected>[[[All]]]</option>' + childItems).selectpicker("refresh");
...
$(document).on("change", "#selected_childdashboard", function () {
    ...
    if (selectedValue.toLowerCase() == "all" || selectedValue.toLowerCase() == "") {
...
function partialPost(url, className) {
    if (!$(".schedule-dialog").hasClass(className)) {
        ...
        $.ajax({
                ...
                if (className == "subscribers-panel") {
                    ...
                    if (actionType == "Create") {
                        $("select#user-search option").each(function (i) {
                            if ($(this).val().toLowerCase() == $("#userName").val().toLowerCase()) {
                                ...
                                $("#noatch-export").removeAttr("disabled");
                                $("#noatch-export-lable").removeClass("disabled-lable-color");
                                if (exportFormat.CanExportImage) {
                                    $("#image-export").removeAttr("disabled");
                                    $("#image-export-lable").removeClass("disabled-lable-color");
                                }
                                if (exportFormat.CanExportPdf) {
                                    $("#pdf-export").removeAttr("disabled");
                                    $("#pdf-export-lable").removeClass("disabled-lable-color");
                                }
                                if (exportFormat.CanExportExcel) {
                                    $("#excel-export").removeAttr("disabled");
                                    $("#excel-export-lable").removeClass("disabled-lable-color");
                                }
                            }
                        });
                    }
                        ...
                        $("#noatch-export").prop("checked", exportType.toLowerCase() == "noattachment");
                    ...
                    if (actionType == "Create") {
                        ...
                        if (emailContent == null) {
                            ...
                            var data = emailContent.match(/\{\:(.*?)\}/g);
                            for (i = 0; i < data.length; i++) {
                                if (!(data[i] === "{:Username}" || data[i] === "{:OrganizationName}")) {
                                    for (t = 0; t < selectedColumns.length; t++) {
                                        if (selectedColumns.eq(t).val() != "undefined" && selectedColumns.eq(t).val() != null) {
                                            if (data[i].toLocaleLowerCase().indexOf(selectedColumns.eq(t).val().toLocaleLowerCase()) >= 0 !== true) {
                                                emailContent = emailContent.replace(data[i], "");
                                            }
                                        }
                                    }
                                }
            ...
            complete: function () {
                if (className == "email-editor-panel") {
                    setTimeout(function () {
                        cursorPos.codemirror.refresh();
                    }, 10);
                }
            ...
            success: function (data) {
                ...
                if (actionType == "Edit") {
                    if (item.ItemName != null) {
                        ...
                        renderDataCondition(itemConditionCategory, itemConditionArray, itemWidgetName.trim(), item);
                        addTitleForWidgets();
                    }
                    else {
                        getWidgets();
...
function editSchedule(id, itemId, itemName, categoryName, multidashboardname) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (exportType.CanExportImage) {
                $("#image-export").prop("checked", true);
            }
            else if (exportType.CanExportPdf) {
                $("#pdf-export").prop("checked", true);
            }
            else if (exportType.CanExportExcel) {
...
$(document).on('show.bs.dropdown', '.childdashboard-dropdown', function () {
    if (actionType == "Edit" && $("#selected_childdashboard").find("option").length <= 1) {
        ...
        $("#selected_childdashboard").html("");
        $("#selected_childdashboard").append('<option value="All">[[[All]]]</option>' + childItems).selectpicker("refresh");
        addTitleForChildDashboards();
