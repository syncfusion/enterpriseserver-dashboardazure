...
$(document)
    .ready(function () {
        ...
        $(".custom-expression-editor-container").ejWaitingPopup();
        if (columns != undefined) {
            ...
            if (expressionList != undefined) {
                for (var i = 0; i < expressionList.Data.Result.length; i++) {
                    listItems += '<option value="' + expressionList.Data.Result[i].Name + '" data-value="' + expressionList.Data.Result[i].ExpressionId + '" >' + expressionList.Data.Result[i].Name + '</option>';
                    $("#selected-column-value").html(listItems).selectpicker("refresh");
                }

            }
            columlistItems = listItems;
            listItems = "";
        }
        ...
        if (expressionList != undefined) {
            for (var i = 0; i < expressionList.Data.Result.length; i++) {
                listItems += "<li class='items' data-value='" + expressionList.Data.Result[i].Name + "' value='" + expressionList.Data.Result[i].ExpressionId + "' data-expression='" + encodeHTML(expressionList.Data.Result[i].Expression) + "'>" +
        ...
        $("#customexpression-delete-confirmation").ejDialog({
            width: "450px",
            showOnInit: false,
            ...
            showHeader: false,
            enableModal: true,
            close: "onDeleteExpressionDialogClose",
            open: "onDeleteExpressionDialogOpen"
        });

        $("#customexpression-delete-confirmation_wrapper").ejWaitingPopup();
});
...
$(document).on("click", "#save-expression",
    function () {
        ...
        if ($("#expression-name-value").val() == "") {
            ...
            $("#expression-name-validation").html("[[[Please enter expression name.]]]");
        ...
        if ($("#rte-post").val() == "") {
            ...
            $("#expression-text-validation").html("[[[Please enter expression text.]]]");
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.toLowerCase() == "false") {
                                ...
                                if (data.Result == true) {
                                    for (var i = 0; i < $("#list-items").find(".items").length; i++) {
                                        if ($($("#list-items").find(".items")[i]).val() == expressionId) {
                                            ...
                                            $($("#list-items").find(".items")[i]).attr("data-expression", expressionText)
                                        }
                                    }
                                    for (var i = 0; i < $("#selected-column-value").find("option").length; i++) {
                                        if ($($("#selected-column-value").find("option")[i]).attr("data-value") == expressionId) {
                                            $($("#selected-column-value").find("option")[i]).text(expressionName);
                                            $("#selected-column-value").selectpicker("refresh");
                                        }
                                    }
                                    $("#expression-text-validation").html("[[[Expression updated successfully.]]]");
                                    ...
                                    SuccessAlert("[[[Expression updated]]]", "[[[Expression has been updated successfully]]]", 5000);
                                    $("#expression-text-validation").html("");
                                } else {
                                    $("#rte-post").addClass("has-error");
                                    $("#expression-text-validation").addClass("validate-error");
                                    if (data.ErrorDetails != null) {
                                        ...
                                        $("#expression-text-validation").html("[[[Invalid expression]]]");
                                    }
                                }
                                $(".custom-expression-editor-container").ejWaitingPopup("hide");
                            }
                        ...
                        $("#expression-name-value").addClass("has-error");
                        $("#expression-name-validation").html("[[[Expression name already exists.]]]");
                        $("#expression-name-validation").addClass("validate-error");
                        $(".custom-expression-editor-container").ejWaitingPopup("hide");
                        return false;
                    }
                            ...
                            success: function(data) {
                                if (data.Result == true) {
                                    $.ajax({
                                        ...
                                        success: function(data) {
                                            if (data.Result != null) {
                                                listItems = "<li class='items' data-value='" + expressionName + "' value='" + data.Result.ExpressionId + "' data-expression='" + encodeHTML(data.Result.Expression) + "'>" +
                                                    expressionName +
                                                    "<span class='su-close hidden'></span></li>";
                                            ...
                                            columlistItems = '<option value="' + expressionName + '"data-value ="' + data.Result.ExpressionId + '">' + expressionName + '</option>';
                                            $("#selected-column-value").find("option:last").after(columlistItems);
                                            $("#selected-column-value").selectpicker("refresh");
                                            $(".custom-expression-editor-container").ejWaitingPopup("hide");
                                        }
                                    });
                                    SuccessAlert("[[[Expression saved]]]", "[[[Expression has been saved successfully]]]", 5000);
                                    $("#expression-text-validation").html("");
                             } else {
                                    $("#rte-post").addClass("has-error");
                                    $("#expression-text-validation").addClass("validate-error");
...
$(document)
    .on("click",
        ...
        function(e) {
            ...
            $("#customexpression-delete-confirmation").ejDialog("open");        
            DeleteExpression(userId, datasourceId, expressionName);
            
        });



$(document).on("click", "#delete-expression", function (e) { 
    var expressionName = $(this).attr("data-expression-name");
    var datasourceId = $(this).attr("data-datasource-id");
    var userId = $("#userid").val();
     deleteExpressionList.push(expressionName);
    $.ajax({
        type: "POST",
        url: DeleteExpressionUrl,
        data: { userId: userId, datasourceId: datasourceId, expressionName: expressionName, deleteExpressionList: deleteExpressionList },
        success: function (data) {
            if (data.toLowerCase() == "true") {
                for (var j = 0; j < deleteExpressionList.length; j++) {
                for (var i = 0; i < $("ul li.items").length; i++) {                   
                    if ($($("ul li.items")[i]).text().toLowerCase() == deleteExpressionList[j].toLowerCase()) {
                ...
                for (var j = 0; j < deleteExpressionList.length; j++) {
                    for (var i = 0; i < $("#selected-column-value").find("option").length; i++) {
                        if ($($("#selected-column-value").find("option")[i]).attr("value").toLowerCase() == deleteExpressionList[j].toLowerCase()) {
                            $($("#selected-column-value").find("option")[i]).remove();
                            $("#selected-column-value").selectpicker("refresh");
                        }
                    }
                }
                for (var i = 0; i < deleteExpressionList.length; i++) {
                   ...
                   .each(function () {
                       for (var j = 0; j < parent.$(".measure-column select option,.measure-column-col select option").length; j++) {
                           if ($(parent.$(".measure-column select option,.measure-column-col select option")[j]).attr("value").toLowerCase() == deleteExpressionList[i].toLowerCase()) {
                    ...
                    parent.$(".dimension-column select option,.dimension-column-col select option")
                        .each(function () {
                            for (var i = 0; i < parent.$(".dimension-column select option,.dimension-column-col select option").length; i++) {
                                if ($(parent.$(".dimension-column select option,.dimension-column-col select option")[i]).attr("value").toLowerCase() == deleteExpressionList[j].toLowerCase()) {
                ...
                for (var j = 0; j < deleteExpressionList.length; j++) {
                    if ($("#expression-name-value").val().toLowerCase() == deleteExpressionList[j].toLowerCase()) {
                        $("#expression-name-value").val('');
                        $("#rte-post").val('');
                    }
                }
               
            SuccessAlert("[[[Expression deleted]]]", "[[[Expression has been deleted successfully]]]", 5000);
            $("#expression-text-validation").html("");
            $("#customexpression-delete-confirmation").html("");
            $("#customexpression-delete-confirmation").ejDialog("close");
            }
            else {
                WarningAlert("[[[Delete Expression]]]", "[[[Error while deleting the expression.Please try again.]]]", 5000);
            }
...
function DeleteExpression(userId, datasourceId, expressionName, targetElement) {
    var expressionNameList = [];
    expressionNameList.push(expressionName);
    $.ajax({
        type: "POST",
        url: DeleteCustomExpressionConfirmationUrl,
        data: { datasourceId: datasourceId, userId: userId, expressionNameList: expressionNameList, expressionName: expressionName },
        success: function (data) {
            if (data) {
                $("#customexpression-delete-confirmation").html(data);
            }
        }
    });
}
...
function onDeleteExpressionDialogClose() {
    $("#customexpression-delete-confirmation").html("");
    $("#customexpression-delete-confirmation").ejDialog("close");
}

function onDeleteExpressionDialogOpen() {
    $("#customexpression-delete-confirmation").ejDialog("open");
    $('#customexpression-delete-confirmation').focus();
}

function encodeHTML(expression) {
