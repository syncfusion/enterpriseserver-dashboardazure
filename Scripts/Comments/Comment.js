var cursorPos, Paste, createHiddenEditable, isFocusable, initialLine, initialPosition, targetElement, itemId, dataManger, elementNode, isMultiDashboard, getAllCommentAjax;
...
$(document).ready(function () {
    ...
    $(document).on('click', '#post-comment', function () {
        ...
        if (currentPageUrl.indexOf("unlistedCode") > 0) {
            var substring = currentPageUrl.substring(currentPageUrl.indexOf("unlistedCode") - 1, currentPageUrl.indexOf("unlistedCode") + 23);
            currentPageUrl = currentPageUrl.replace(substring, "");
        }
        ...
        if (itemType.toLowerCase() == 'dashboard') {
            ...
            querystring != "" ? currentPageUrl = "/" + splitUrl[1] + "/view?id=" + dashboardItemId + "&" + querystring : currentPageUrl = "/" + splitUrl[1] + "/view?id=" + dashboardItemId;
        }
        if (itemType.toLowerCase() == 'widget') {
            ...
            if ($('#isWidgetComment').val() == "true") {
                ...
                var splitUrl = currentPageUrl.split("view");
                currentPageUrl = splitUrl[0] + "view?id=" + itemId;
            }
        }
        if (isMultiDashboard.toLowerCase() == 'true') {
            var splitUrl = currentPageUrl.split("view");
            querystring != ""
                ? currentPageUrl = splitUrl[0] + "view?id=" + parentDashboardId + "&" + querystring
                : currentPageUrl = splitUrl[0] + "view?id=" + parentDashboardId + '&tab=' + parent.$("#dashboard").find("li.e-active").find("span").html().trim();
        }
        var commentText = postmd.value();
        ...
        if (commentText.length > 4000) {
            ...
            $(".comment-area").val("");
            $(".error-message").html("");
            var values = { itemId: itemId, commentText: commentText, itemType: itemType, dashboardItemId: dashboardItemId, url: decodeURI(currentPageUrl), uniqueUsersValue: userMention.uniqueUsersValue, uniqueGroupValue: userMention.uniqueGroupValue, mentionedAll: userMention.mentionedAll, isMultiDashboard: isMultiDashboard, parentDashboardId: parentDashboardId };
                ...
                success: function (data, result) {
                    if (data) {
                            var addComment = "<li id='comment-"
                                ...
                                + data.Comments[0].UserDisplayName + "'class='author publisher-anchor-color' data-toggle='tooltip' data-placement='right'>"
                                + data.Comments[0].UserDisplayName + "</span><span class='su su-status-active'></span></span><menu><li class='reply' data-toggle='tooltip' data-placement='left' title='[[[Reply]]]'> <a class='reply-comment'><span class='su su-reply'></span> </a></li><span class='dropdown'><span class='dropdown-toggle option-icon' title='[[[Actions]]]' data-toggle='dropdown'data-placement='left'><span class='su su-options-horizontal'></span></span><ul class='dropdown-menu' role='menu'><li class='edit' data-toggle='tooltip' data-placement='top' title='[[[Edit]]]'> <a class='edit-comment'><span>[[[Edit]]]</span> </a></li><li class='delete' data-toggle='tooltip' data-placement='top' title='[[[Delete]]]'> <a class='delete-comment'><span>[[[Delete]]]</span> </a></li></menu><span class='post-meta col-xs-9 no-padding'><a href='#comment-"
                                ...
                                + data.Comments[0].TimeAgo + "</span></a></span></span> </header> <div class='post-body-inner'><div class='edit-post displayNone'></div><div class='post-message'>"
                            ...
                            $('span.dropdown-toggle.option-icon').tooltip();
                            postmd.value("");
                            $("#comment_action_area").show();
                            ...
                            if (cmntCount == 0) {
                                if (itemType == "dashboard") {
                                    parent.$('#dashboard').data("ejDashboardViewer").toggleDashboardComment();
    ...
    $(document).on('click', '.edit-comment', function () {
        ...
        $("#save-edit").on('click', function () {
            ...
            var values = {
                ...
                isMultiDashboard: isMultiDashboard,
                ...
                $.ajax({
                    ...
                    success: function (data, result) {
                        if (data) {
                            ...
                            addComment += "<li class='reply' data-toggle='tooltip' data-placement='left' title='[[[Reply]]]'> <a class='reply-comment'><span class='su su-reply'></span> </a></li><span class='dropdown'><span class='dropdown-toggle option-icon' title='[[[Actions]]]' data-toggle='dropdown' data-placement='left'><span class='su su-options-horizontal'></span></span><ul class='dropdown-menu' role='menu'><li class='edit' data-toggle='tooltip' data-placement='top' data-toggle='tooltip' data-placement='top' title='[[[Edit]]]'> <a class='edit-comment'><span>[[[Edit]]]</span> </a></li><li class='delete' data-toggle='tooltip' data-placement='top' title='[[[Delete]]]'> <a class='delete-comment'><span>[[[Delete]]]</span> </a></li><li class='parent' data-parent-id='"
                                + data.Comments[0].ParentId + "' data-toggle='tooltip' data-placement='top' title='[[[Replied to]]] "
    ...
    $(document).on('click', '.delete-comment', function () {
        ...
        var values = { itemId: itemId, commentId: commentId, itemType: itemType, dashboardItemId: dashboardItemId, url: decodeURI(currentPageUrl), isMultiDashboard: isMultiDashboard, parentDashboardId: parentDashboardId };
        var deletedcomment = "";
        parent.$("#delete_popup_iframe").attr("src", commentDeleteUrl);
        parent.$("#delete_popup_iframe").removeClass("displayNone");
        parent.$("#delete_popup_iframe").on('load',function () {
            parent.frames[1].messageBox("su-delete", "[[[Delete Comment]]]", "[[[Are you sure you want to delete this comment?]]]", "error", function () {
    ...
    $(document).on('click', ".hasChild", function () {
        if ($(this).find('.collapse-icon').hasClass("su-vertical-expand")) {
            ...
            $(this).find('.collapse-icon').attr({ 'data-original-title' :'[[[Collapse]]]', 'data-toggle' : 'tooltip', 'data-placement' : 'left'});
        } else {
            ...
            $(this).find('.collapse-icon').attr({ 'data-original-title': '[[[Expand]]]', 'data-toggle': 'tooltip', 'data-placement': 'left' });
        }
        $(this).find('.collapse-icon').tooltip();
    });
    ...
    $(document).on('click', ".post-message > p > img", function () {
        ...
        var windowWidth = parent.$("#dashboard").width() == null ? parent.$("#widget").width() : parent.$("#dashboard").width();
...
function GetAllComments(itemId, itemType, dashboardItemId, orderBy, isMultiDashboard) {
    ...
    var skip = 0;
    var take = 0;
    var values = "&itemId=" + itemId + "&orderBy=" + orderBy + "&skip=" + skip + "&take=" + take + "&itemType=" + itemType + "&dashboardItemId=" + dashboardItemId + "&isMultiDashboard=" + isMultiDashboard + "&parentDashboardId=" + parentDashboardId;
    showWaitingPopup("parent-scroll-element");
        ...
        success: function (data, result) {
            if (data) {
                if (data.Status == true) {
                    ...
                    if (!parent.$("#commentModuleContainer").hasClass("displayNone") || !parent.$("#widgetCommentModuleContainer").hasClass("displayNone")) {
...
function refreshCommentScroller() {
    ...
    if ($('#isWidgetComment').val() == 'true' && itemId !== "undefined") {
        ...
        var positionY = parent.$('#' + itemId).offset().top;
    ...
    if (!($('#isWidgetComment').val() == 'true')) {
        ...
        $("#scroll-element").children().addClass('e-content');
        var scrollerHeight = parent.$("#commentModuleContainer").height() - (textAreaHeight + $('.count-sort').outerHeight());
        ...
        for (var i = 0; i < item.length; i++) {
            ...
            if (!item[i].IsActive) {
                ...
                commentHtml += "<li id='comment-"
                    ...
                    + item[i].UserDisplayName + "'class='author publisher-anchor-color'data-toggle='tooltip' data-placement='right'>"
                    + item[i].UserDisplayName + "</span></span><menu>";
                commentHtml += "<li class='reply' data-toggle='tooltip' data-placement='left' title='[[[Reply]]]'> <a class='reply-comment'><span class='su su-reply'></span> </a></li>";

                if ($("#isadmin").text().toLowerCase() == "true" || item[i].UserId == $("#userName").val() || parentItem != null) {
                    commentHtml += "<span class='dropdown' id='actions-menu'><span class='dropdown-toggle option-icon' title='[[[Actions]]]' data-toggle='dropdown' data-placement='left'><span class='su su-options-horizontal'></span></span><ul class='dropdown-menu' role='menu'></span>";
                    if (item[i].UserId == $("#userName").val()) {
                        commentHtml += "<li class='edit' data-toggle='tooltip' data-placement='top' title='[[[Edit]]]'> <a class='edit-comment'><span>[[[Edit]]]</span> </a></li>";
                    }
                    if ($("#isadmin").text().toLowerCase() == "true" || item[i].UserId == $("#userName").val()) {
                        commentHtml += "<li class='delete' data-toggle='tooltip' data-placement='top' title='[[[Delete]]]'> <a class='delete-comment'><span>[[[Delete]]]</span> </a></li>";
                    }
                    if (parentItem != null) {
                        commentHtml += "<li class='parent' data-parent-id='"
                        + parentItem.Id + "' data-toggle='tooltip' data-placement='top' title='[[[Replied to]]] "
                ...
                if (childCount > 0) {
                    commentHtml += "su-vertical-collapse collapse-icon' data-toggle='tooltip' data-placement='left' title='[[[Collapse]]]'></span>";
                } else {
...
function pasteImageReader (dataURL) {
    ...
    var values = { DataUrl: dataUrl, itemId: itemId, itemType: itemType, isMultiDashboard: isMultiDashboard, parentDashboardId: parentDashboardId };
    $.ajax({
        type: "POST",
        url: commentImageUrl,
        data: values,
...
$(document).on("click", "#close-comment", function () {
    ...
    if (parent.$("#is_mobile").val() == "true") {
        ...
        if (parent.$("#server-mobile-navbar .server-comment").hasClass('active')) {
            ...
            parent.$("#dashboard, #widget").show();
            parent.$("#commentModuleContainer").css("z-index", "");
        }
    }
    ...
    if (iframeUrl == parentUrl) {
        parent.$("#dashboard").css("width", $(parent.window).width() - 40 + "px");
        var data = parent.$("#dashboard").data("ejDashboardViewer");
        if (data != null) {
        ...
        if (!parent.$(".su-sidebar-collapse").is(":visible")) {
            parent.$("#dashboard").css("width", parent.document.body.clientWidth - 40);
...
$(document).on('show.bs.dropdown', function (e) {
    var eventTargetElement = $(e.target);
    var availableTopHeight = eventTargetElement.find(".option-icon").offset().top + eventTargetElement.find(".option-icon").outerHeight(true) / 2 - $(".header-section").outerHeight(true) - $("#comment_action_area").outerHeight(true) - $(".col-xs-12 count-sort").outerHeight(true);
    var availableBottomHeight = $(window).height() - eventTargetElement.find(".option-icon").offset().top + eventTargetElement.find(".option-icon").outerHeight(true) / 2;
    var dropDownHeight = eventTargetElement.find(".dropdown-menu").outerHeight(true);
    if (availableBottomHeight <= dropDownHeight && availableTopHeight >= dropDownHeight) {
        eventTargetElement.removeClass("dropdown").addClass("dropup");
    }
});