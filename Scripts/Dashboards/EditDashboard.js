﻿$(function () {
    ...
    $(document).on('change', '#browse_file', function (e) {
        if ($(this).val().substring($(this).val().lastIndexOf('.') + 1) == "sydx" ||
            ($(this).val().substring($(this).val().lastIndexOf('.') + 1) == "sydj" && isNewViewerModel.toLowerCase() == "true")) {
            ...
            if (isNewViewerModel.toLowerCase() == "true") {
                $("#validate-file").text("[[[Please choose a valid SYDX or SYDJ file to upload]]]");
            } else {
...
function validateNewFileForm() {
    ...
    if (canProceed) {
        var oldExtension = $("#file-extension").attr("data-extension");
        var newExtension = $("#name").val().substring($("#name").val().lastIndexOf('.'));
        if (oldExtension == ".sydx" && newExtension == ".sydj" && isNewViewerModel.toLowerCase() == "true") {
            parent.messageBox("su-nav-dashboard", "[[[Edit Dashboard]]]", "[[[Editing this dashboard will upgrade it to a newer format which is currently not compatible with the following scenarios:]]]<br/><ul class='message-content'><li>[[[Embedding dashboards using Dashboard Platform SDK]]]</li><li>[[[Sending data alerts]]]</li><li>[[[Pinning dashboards/widgets]]]</li></ul>[[[Do you want to continue?]]]", "error", function () {
                parent.$("#edit-file-popup_wrapper").ejWaitingPopup("show");
                $('form').submit();
            }, parent.$("#edit-file-popup_wrapper").ejWaitingPopup("hide"), "auto", "auto", undefined,"upgrade_dashboard_sydj");
        }
        else {
            $('form').submit();
        }
