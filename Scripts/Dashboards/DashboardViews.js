...
$(document).ready(function () {
    ...
    $("#save-view-popup_wrapper").ejWaitingPopup();
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.StatusMessage.trim().toLowerCase() == "name already exists") {
                    ...
                    messageBox("su-filter", '[[[Save View]]]', "[[[View has been saved successfully.]]]", "success", function () {
                        onCloseMessageBox();
            ...
            error: function (data) {
                messageBox("su-filter", '[[[Save View]]]', "[[[Internal server error. Please try again.]]]", "success", function () {
                    onCloseMessageBox();
                });
                $("#dashboard-view-toogle").ejWaitingPopup("hide");
                parent.$("#save-view-popup_wrapper").ejWaitingPopup("hide");
...
function GetSavedFilter() {
    ...
    $.ajax({
        ...
        type: "POST",
        success: function (data) {
            var isunlisted = data.IsUnlisted;
            var unlistedCode = data.UnlistedCode;
            var createdItemId = data.IsItemOwner;
            var unlistedUrl = (isunlisted) ? "&unlistedCode=" + unlistedCode : "";
            ...
            if (Result.length > 0) {
                ...
                for (var i = 0; i < Result.length; i++) {
                    ...
                    splittedUrl = pageurl.toString().split("/");
...
function resetViewPanel() {
    ...
    $("#dashboard").ejDashboardViewer("instance").model.filterOverviewSettings.showSaveIcon = !($("#is_mobile").val() == "true" || isUserAuthenticated == "false");
        ...
        success: function (data) {
            if (data.Result.Status == true) {
                ...
                if (currentId == $('#dashboard').data("ejDashboardViewer").model.filterOverviewSettings.viewId) {
                    $('#dashboard').data("ejDashboardViewer").updateFilterOverview("Unsaved View", "");
...
function SavedViewHeight() {
    if ($('#saved-list').css('display') != "none") {
        var expandCollpase = iframeUrl == parentUrl ? parent.$("#header-pane") : $("#header-pane");
        ...
        var windowHeight =  iframeUrl == parentUrl? $(parent.window).height() : $(window).height();
...
function refreshScroller() {
    ...
    if ($("#saved-list").length) {
        ...
        if (parent.$("#saved-filter .e-vhandle").length) {
            var element = parent.$("#saved-filter .e-vhandle");
            if (!(element.css('display') == "none")) {
                var height = element.height() + 29;
                element.css("height", height);
            }
...
function getcurrentfilters() {
    if (iframeUrl == parentUrl) {
        var dashboardviewerObj = parent.$('#dashboard').length > 0 ? parent.$('#dashboard').data("ejDashboardViewer").getCurrentFilters() : $('#dashboard').data("ejDashboardViewer").getCurrentFilters();
    } else {
...
function messageBox(messageIcon, messageHeader, messageText, type, successCallback, errorCallback, width, height, maxHeight, cssClass) {
    ...
    if (type == "error") {
        ...
        if (errorCallback != undefined) {
            ...
            closeIcon = $("<span class='su su-close' data-toggle='tooltip' data-placement='left' data-container='body' title='close'></span>");
...
$(document).on('click', '.options li', function () {
    ...
    switch (selected.toLowerCase()) {
        case "filters":
            ...
            $("#filter-view span.view-heading").length == 0 ? $("#filter-view").append("<span class='view-heading'>[[[FILTERS]]]</span>") : "";
            $("#filter-view").show();
            if ($('#dashboard').data("ejDashboardViewer").hasFilterPanel == true) {
                ...
                $("#filter-view p#validation-message").length == 0 ? $("#filter-view").append("<p id='validation-message'>[[[No filters configured.]]]</p>") : "";
                $("#close-filter a").css("display", "block");
                $("#filter-view").css("width", "410px");
            }
            refreshScroller();
...
function SaveFilter(args) {
    ...
    if (args.viewID == null) {
        ...
        $("#save-view-popup-iframe").attr("src", saveViewPopup + "?itemId=" + itemId + "&isMultidashboardId=" + IsMultiDashboard + "&parentId=" + parentDashboardId);
    }
    else {
...
$(document).on("click", ".saved-view", function (e) {
    if (e.target.className == "saved-view" || e.target.className == "saved-opt" || e.target.className == "saved-filter txt-overflow") {
        ...
        if ($(currentTarget).find(".applied-filters").length == 0) {
            ...
            $(currentTarget).append($("<span class='col-sm-4 no-padding loader-gif' style='display:inline; background-image:url(" + rootBaseUrl + "content/styles/essentialjs/images/waitingpopup.gif); background-size:cover; position:absolute; height:25px; width:25px;'></span>"));
            $.ajax({
