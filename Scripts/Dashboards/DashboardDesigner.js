    ...
    */
    ...
    var designerModel = {
        ...
        serverSettings: {
            ...
            warningMessage: isAdmin.toLocaleLowerCase() == "true" ? "Public dashboards are disabled in <a href='/administration/dashboard-settings' target='_blank'> dashboard settings" : "Public Dashboards are disabled by the system administrator. Please contact your system administrator to enable this."
        },
        viewerSettings: {
...
function onSaveDashboard(args) {
    ...
    if (args.eventType == "Save") {
        ...
        updateURL(categoryName, savedDashboardDetails.dashboardName, savedDashboardDetails.dashboardId, savedDashboardDetails.version);
        var newTitle = savedDashboardDetails.dashboardName + '- Design Dashboard - ' + organizationName;
        document.title = newTitle;
        $('.edit-dashboard').html(savedDashboardDetails.dashboardName);
        $('.dashboard-status').html('(Saved)');
