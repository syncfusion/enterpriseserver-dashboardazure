...
$(function () {
    ...
    if ($("#emtpyCategoryList").length == 1) {
        $("#selected_category").removeClass("bottom-margin30").addClass("bottom-margin-validation");
        $('#selected_category').parent('div').css("margin-top", "-11px");
        $("#selected_category").closest("div").addClass("has-error");
    }
    ...
    $(document).on("change", "#browse_file", function () {
        if ($(this).val().substring($(this).val().lastIndexOf('.') + 1) == "sydx" ||
            $(this).val().substring($(this).val().lastIndexOf('.') + 1) == "sydj") {
            ...
            $("#validate-file").html("[[[Please choose a valid SYDX or SYDJ file to upload]]]");
