    ...
    function receiveMessage(event) {
        ...
        if (event.data) {
                    ...
                    $.ajax({
                        ...
                        success: function (obj) {
                            if (obj.data != null && obj.data.Status == true) {
                                ...
                                if ($("#dashboard").data("ejDashboardDesigner") != null) {                                  
                                    $("#dashboard").data("ejDashboardDesigner").model.filterOverviewSettings.showSaveIcon = (view == "true") ? true : false;
                                    $("#dashboard").data("ejDashboardDesigner").model.filterOverviewSettings.showSaveAsIcon = (view == "true") ? true : false;
                                    $("#dashboard").data("ejDashboardDesigner").model.filterOverviewSettings.showViewSavedFilterIcon = (view == "true") ? true : false;
                                    if ($("#dashboard").data("ejDashboardDesigner").model.enableComment == "false" && enableComment == "true") {
                                        $("#dashboard").data("ejDashboardDesigner")._setModel({ "enableComment": (comment == "true") ? "true" : "false" });
                                    }
                                    if (enableComment.toLowerCase() == "true" && ej.DashboardDesignerUtility.Serialization.getInstance().dashboardPropertyJson.EnableComments) {
                                        $("#dashboard-comment").css('display', 'block');
                                        $(".su-with-comment", ".su-without-comment").css('display', 'block');
                                    }
                                    else {
                                        $("#dashboard-comment").css('display', 'none');
                                        $(".su-with-comment", ".su-without-comment").css('display', 'none');
                                    }

                                    if (enableView.toString() == "true") {
                                        $("#dashboard-view").css('display', 'block');
                                    }
                                    else {
                                        $("#dashboard-view").css('display', 'none');
                                    }
                                }
                                else {
                                    designerToken = token;
                                ...
                                if (isPublic == "true" || isUnlisted == "true" && isUserAuthenticated.toString().toLowerCase() == "false") {
                                    enableComment = "false";
                                    enableView = "false";
                                    renderDashboard();
                                } else {
                                    CustomErrorMessage(obj.data.ErrorCode, itemType.toString());
            ...
            if (isMarkItemsPublic == "false" && isItemPublic == "true" && isUnlisted != "true") {
                CustomErrorMessage(publicItemSetttingOffWithoutLoginAndWithOutAccess, itemType.toString());
            }
            else if (isMarkItemsPublic == "true" && isItemPublic == "false" && isUnlisted != "true") {
                CustomErrorMessage(privateItemSetttingOnWithoutLoginAndWithOutAccessForWinAuth, itemType.toString());
            }
            else if (isMarkItemsPublic == "false" && isItemPublic == "false" && isUnlisted != "true") {
                CustomErrorMessage(privateItemSetttingOffWithoutLoginAndWithOutAccessForWinAuth, itemType.toString());
            }
        }
    }
    if (externalLogin != "" && externalLogin.toLowerCase() == "windowsad" && isUserAuthenticated.toString().toLowerCase() == "true") {
        if (isPublic == "true" || isUnlisted == "true") {
            ...
            CustomErrorMessage(publicItemSetttingOffWithLoginAndWithOutAccess, itemType.toString());
        }
        else if (isMarkItemsPublic == "true" && isItemPublic == "false" && canRead.toLowerCase() == "false" && isUnlisted != "true") {
            CustomErrorMessage(privateItemSetttingOnWithLoginAndWithOutAccess, itemType.toString());
        }
        else if (isMarkItemsPublic == "false" && isItemPublic == "false" && canRead.toLowerCase() == "false" && isUnlisted != "true") {
            CustomErrorMessage(privateItemSetttingOffWithLoginAndWithOutAccess, itemType.toString());
        }
        else {
            window.location.href = dashboardServerUrl + "/login";
        }
    }
    if (externalLogin != null && externalLogin.toLowerCase() == "azuread" && sso != null && sso.toLowerCase() == "true") {
        if (isUserAuthenticated.toString().toLowerCase() == "true") {
    ...
    if (sso.toLowerCase() == "false") {
        if (isUserAuthenticated.toString().toLowerCase() == "true") {
            ...
            if (isPublic == "true" || isUnlisted == "true" && isUserAuthenticated.toString().toLowerCase() == "false") {
                ...
                if (isUserAuthenticated.toString().toLowerCase() == "false" && designerToken != "") {
                    enableComment = "false";
                    enableView = "false";
                    renderDashboard();
                    $("#dashboard").data("ejDashboardDesigner").model.serviceAuthorizationToken = designerToken;
                }
            }
        }
    }
    ...
    if (browser != null && browser.name === "msie" && parseFloat(browser.version) <= 8.0) {
        ...
        if (viewId != null && viewId != "") {
                ...
                success: function (data) {
                    ...
                    $('#dashboard').ejDashboardDesigner({
                        ...
                        filterOverviewSettings: {
                            ...
                            showSaveAsIcon: (isSlideshow) ? false : ($("#is_mobile").val() == "false" ? isUserAuthenticated.toString().toLowerCase() == "true" : false),
            ...
            $('#dashboard').ejDashboardDesigner({
                ...
                _onFavoriteStateChange: "updatefavorite",
                filterOverviewSettings: {
                    showSaveIcon: (isSlideshow) ? false : ($("#is_mobile").val() == "false" ? isUserAuthenticated.toString().toLowerCase() == "true" : false),
                    showSaveAsIcon: (isSlideshow) ? false : ($("#is_mobile").val() == "false" ? isUserAuthenticated.toString().toLowerCase() == "true" : false),
                    showViewSavedFilterIcon: !(isSlideshow),
                    viewName: null,
                    viewId: null
...
function onActionCompleteOfNewDashboardViewer(arg) {
    ...
    if (enableComment.toLowerCase() == "true" && ej.DashboardDesignerUtility.Serialization.getInstance().dashboardPropertyJson.EnableComments) {
        $(".su-with-comment").removeClass('displayNone');
        $(".su-without-comment").removeClass('displayNone');
    }
    else {
        $(".su-with-comment").addClass('displayNone');
        $(".su-without-comment").addClass('displayNone');
    }
}
