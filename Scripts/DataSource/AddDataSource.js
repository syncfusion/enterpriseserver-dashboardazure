$(function () {
    ...
    $(document).on("change", "#browse-file", function () {
        ...
        if ($("#file-name-content").val() == 0) {
            $("#file-name-content").val(name.substring(0,name.lastIndexOf('.')));
            $("#add-item-form").valid();
        }
       });
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.Data) {
                    ...
                    $("#validate-name").text("[[[Data source name already exists]]]");
...
function validFileType() {
    if ($("#browse-file").val() != '') {
        if ($("#file-name").val().split(".").pop().toLowerCase() !== "syds" && $("#file-name").val().indexOf(".") > -1 && $("#file-name").val() != '') {
            ...
            $("#file-name").addClass('error-file-upload').val("[[[Browse file path]]]");
            $("#validate-file").html("[[[Please choose a valid SYDS file to upload]]]");
            $("#browse-file").val("");
            return false;
        }
        else {
        ...
        $("#file-name").closest('td').find("span.validation-message").html("[[[Please upload data source file]]]");
        return false;
    }
}
...
function updateContents(itemId, name) {
    ...
    parent.document.getElementById('update-data-source-popup-iframe').contentWindow.changedValues.push({ DataSourceId: itemId, Name: name });
    parent.document.getElementById('update-data-source-popup-iframe').contentWindow.updateDatasource(parent.$("#update-data-source-popup-iframe").contents().find("select.update-datasource.current-select"));
}