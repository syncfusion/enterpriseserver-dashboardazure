...
$(function () {
    ...
    $(document).on("click", ".version-button", function () {
        ...
        var dataManager_version = ej.DataManager({ url: versionItemsUrl + "?ItemId="+ itemId, adaptor: new ej.UrlAdaptor() });
    ...
    $(document).on("click", ".item-permissions", function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var dataManager_users = ej.DataManager({ url: getItemUserPermissionUrl + "?ItemId=" + itemId, adaptor: new ej.UrlAdaptor() });
                userPermission.dataSource(dataManager_users);
                userPermission.model.pageSettings.currentPage = 1;
                var dataManager_groups = ej.DataManager({ url: getItemGroupPermissionUrl + "?ItemId=" + itemId, adaptor: new ej.UrlAdaptor() });
    ...
    $(document).on("click", ".get-link", function (e) {
        ...
        if (iframe != null) {
            ...
            if ($(innerDoc).find("#favorite_Item").length > 0 && document.getElementById("dashboard_render_iframe").contentWindow.getcurrentfilters() !== null) {
                if (document.getElementById("dashboard_render_iframe").contentWindow
                    .getcurrentfilters().encryptedData != null) {
    ...
    if (gridName.toLowerCase() == "dashboards" || gridName.toLowerCase() == "widgets") {
        ...
        if ($("ul.item-navigation li.active").length > 0 && $("ul.item-navigation li.active").attr("data-section").toLowerCase() == 'draft dashboards') {
            $(".item-link").each(function () {
                $(this).attr("href", designerUrl + "/" + $(this).attr("data-itemid") + "/draft/" + $(this).attr("data-name"));
            });
        }
        else if (gridName.toLowerCase() == "dashboards" && window.innerWidth >= 1041) {
            $(".item-link").each(function () {
                $(this).attr("href", fileRenderUrl + "?category=" + $(this).attr("data-category-name") + "&dashboard=" + $(this).attr("data-name") + "&view=" + $(".item-navigation li.active").attr("data-value"));
            });
        }
...
$(document).on("click", ".category-collapse", function (e) {
    if ($(".su-sidebar-collapse").is(":visible")) {
        ...
        $("#dashboard_render_iframe").contents().find("#dashboard").css("width", windowwidth - $("#dashboard_render_iframe").contents().find("ul.options").outerWidth());       
    ...
    if (loadViews == "true") {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (isDropupViews) {
                    var liCount = element.children().length;
                    var liHeight = (liCount * element.children().height()) + 10;
                    element.css("height", liHeight + "px");
                }
...
$(document).on("mouseleave", ".dashboard-views", function () {
...
$(document).on("click", ".web-designer", function (e) {
    ...
    if (extension == ".sydx" && isNewViewerModel.toLowerCase() == "true") {
