        ...
        if (parent.window.IsPinWidget && itemType == "widget") {
            ...
            if (homepageItemId != "" && widgetItemId != "") {
                $.ajax({
                    ...
                    success: function (result) {
                        if (result.Success) {
                            SuccessAlert("[[[Pin Widget to the Homepage]]]", "[[[Widget has been pinned successfully.]]]", 7000);
                            $.ajax({
                                type: "POST",
                            ...
                            WarningAlert("[[[Pin Widget to the Homepage]]]", "[[[Unable to pin widget to the homepage. Widget limit exceeded.]]]", 7000);
                        }
                        else {
                            WarningAlert("[[[Pin Widget to the Homepage]]]", "[[[Error while pinning Widget to the homepage.]]]", 7000);
                        }
                    }
                });
            }
            ...
            if (parent.window.IsPinDashboard && itemType == "dashboard") {
                var gridType = $("#items").attr("data-grid-type").toLowerCase();
                    ...
                    parent.homepageActionPopup("newdashboardhomepage");
            }
        }
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.Success) {
                ...
                WarningAlert("[[[Preview Item]]]", "[[[Internal server error. Please try again.]]]", 7000);
            }
        }
    });
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.Success) {
                    ...
                    WarningAlert("[[[Pin Dashboard to the Homepage]]]", "[[[Error while getting tabs for multi-tabbed dashboards.]]]", 7000);
                }
            }
        });
