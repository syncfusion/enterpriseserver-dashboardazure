    ...
    if (itemDetail.ItemType.toLowerCase() == "widget") {
                ...
                $.each(column[i].Item, function (j) {
                    if (column[i].Item[j].ItemExtension.toLowerCase() !== ".sydj") {
                        $("#widget_" + (i + 1) + "_" + (j + 1)).ejDashboardViewer({
                            ...
                            enableExport: true,
                            enablePrint: false,
                            accessToken: (column[i].Item[j].IsHavingPermission) ? token : null,
                            ...
                            beforeWidgetIconRendered: $.proxy(function (args, event) {
                                ...
                                $("#widget_" + (i + 1) + "_" + (j + 1)).ejDashboardViewer().resize();
                            }, this, column),
                            dashboardCreated: function (args) {
                                var href = $("#widget_" + (i + 1) + "_" + (j + 1)).attr("data-dashboardurl");
                                var hrefElement = 'href="' + href + '"';
                                $("#widget_" + (i + 1) + "_" + (j + 1)).find(".su-open-link-newtab").wrap($('<a class="redirect"' + hrefElement + ' target="_blank">'));
                            },
                            onMenuIconClick: function (information) {
                                ...
                                else if (information.name.toLowerCase() == "applied filters") {
                        ...
                        $("#widget_" + (i + 1) + "_" + (j + 1)).addClass("sydj-format").append("<div class='no-permission'><span class='message'>[[[Currently this widget is not supported.]]]</span></div>");
                    }
                });
            }
            else {
        ...
        if (column[0].Item[0].IsActive && column[0].Item[0].IsHavingPermission && column[0].Item[0].ItemExtension.toLowerCase() !== ".sydj") {
            $("#dashboard_1_1").ejDashboardViewer({
                ...
                _itemId: column[0].Item[0].ItemId,
            ...
            if (column[0].Item[0].IsActive && column[0].Item[0].IsHavingPermission && column[0].Item[0].ItemExtension.toLowerCase() === ".sydj") {
                $("#dashboard_1_1").append("<div class='no-permission'><span class='message'>[[[Currently this dashboard is not supported.]]]</span></div>");
            }
        ...
        $.each(column, function (i) {
            ...
            if (column[i].Item.length > 0) {
                $.each(column[i].Item, function (j) {
                    ...
                    var href = column[i].Item[j].TabId == null ? item + '/view?id=' + column[i].Item[j].ItemId + queryString : item + '/view?id=' + column[i].Item[j].ItemId + '&tab=' + column[i].Item[j].TabId + queryString;
                    $('#column-' + (i + 1)).append('<li class="list-item"><div class="widget" id=widget_' + (i + 1) + '_' + (j + 1) + ' data-dashboardurl="' + href + '" style="height:100%"></div></li>');
                });
            }
            else {
...
function unPinItem(column, position, event) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.Success) {
                $("#column-" + column + " .list-item:eq(" + (position - 1) + ")").remove();
                showEmptyList();
                var isEmptyHomepage = checkEmptyHomepage();
                if (isEmptyHomepage) {
                    $("#widget-container ul li.empty .empty-content").find(".drag-widget").hide();
...
function addToolTip() {
    $("select").each(function () {
        ...
        for (var i = 0; i < $("button[data-id=" + dataId + "]").next().find("ul li").length; i++) {
            ...
            $("button[data-id=" + dataId + "]").next().find(".dropdown-menu.selectpicker li").eq(i).find('a').attr({
                "data-toggle":"tooltip",
