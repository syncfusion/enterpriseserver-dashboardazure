...
$(function () {
    ...
    autoCompleteControlEle.ejAutocomplete({
        ...
        template: getUserGroupProfilePicture("${Name}", "${Type}")
    });
...
function splitUserGroupList() {
    ...
    $.each(selectedItemArr, function (index, item) {
        if (isNaN(item.Name) && item.Type == "user") {
            userEmailList.push(item.Name);
        } else {
            groupIdList.push(item.Name);
        }
    });
}
...
function getUserGroupProfilePicture(Name, Type) {

    return "<div class='col-xs-2 no-padding search-user-group-img-container'><div class='group-logo'><i class='su su-" + Type + "-1'></i></div></div><div class='col-xs-9 no-padding search-user-group-display-name-container'><div class='search-user-group-display-name'><span>" + Name + "</span></div></div>";
}
